...
1. **`data_acquisition`** - For collecting training data from 6 EMG sensors
   ...
   - Streams filtered ADC values over serial at 2000 Hz
...
2. **`real_time_inference`** - For real-time gesture classification
   ...
   - Classifies gestures using filtered TD4 features at 2000 Hz
...
- Runs at 2000 Hz (unified for both data acquisition and real-time inference)
...
1. **Real-Time DSP Filtering** (`prelim_collection()` in [functions.cpp](src/functions.cpp):194-209)
   - Collects **250ms window** of **filtered ADC data** at 2000 Hz
   - **500 samples per sensor** (2000Hz × 0.25s)
...
- Unified 2000 Hz coefficients for both data acquisition and real-time inference
...
// Raw data collection
#define RAW_WINDOW_SIZE   500    // 250ms window (500 samples at 2kHz)
#define SAMPLING_FREQ     2000   // 2000 Hz sampling rate
...
| **Latency** | 250ms | **~262ms (250ms + 12ms filters)** |
...
- ✅ **Unified 2000 Hz sampling**: Same coefficients for all modes
...
- **Unified 2000 Hz sampling**: Same coefficients for all modes (data acquisition and real-time inference)
