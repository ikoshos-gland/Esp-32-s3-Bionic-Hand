...
4. **TFLite Inference** ([main.cpp](src/main.cpp))
   - **Float32** model: features copied with `input->data.f[i]`, probabilities read with `output->data.f[i]`
   - Loads model from `model_tflite` array ([include/model.h](include/model.h)); class names, REST index and
     feature count come from [include/model_meta.h](include/model_meta.h) and are checked with `static_assert`
   - **30KB tensor arena** (weights stay in flash; arena holds activations only)
   - Model: MLP 24 → 256 → 128 → 64 → 11 (float32)
...
- [model.h](include/model.h) + [model_meta.h](include/model_meta.h) - Auto-generated by `export_model_header.py` (float32 weights + class order metadata)
...
- [scripts_ai/](scripts_ai/) - **Reorganized Python ML pipeline** (2025-11-26 update)
  ...
  - 📁 data/ - Auto-generated outputs
    ...
    - `models/` - TFLite models (Float32) and C headers
...
# 6. Train MLP (fixed class order by default, --cv for leave-one-rep-out CV)
python train_test_model.py --cv
# Output: 
#   ../data/models/emg_model_TIMESTAMP.tflite
#   ../data/models/emg_model_TIMESTAMP.h

# 7. Copy BOTH generated headers to include/
cp ../training_outputs/models/mlp_td4_TIMESTAMP_headers/model.h ../../include/
cp ../training_outputs/models/mlp_td4_TIMESTAMP_headers/model_meta.h ../../include/
...
**Quantization:** none (Float32). Int8 requires a newer TFLite Micro port (esp-tflite-micro).
...
cp training_outputs/models/mlp_td4_*_headers/model*.h ../include/
pio run -e real_time_inference -t upload
```
...
- **ESP32Servo** (^3.0.9, single version for all environments) - Servo control for robotic hand
...
- The deployed model is Float32; `.f` accessors are correct. Switch to `.int8` only after moving to an Int8 model.
...
- **Float32 model**: 191 KB in flash, inference well under 20 ms
- **Literature reference**: TD4 + LDA reaches 85-95% in the literature with good electrode placement; this project has not reached that yet (see AUDIT_CHANGES.md)
